...
class HistoryBufferLSTMTD3:
    """
    A LSTM-TD3 agent needs to keep track of its recent (observation,action) pair history.
    This class is used by the replay buffer to keep track of the histories during episodes.
    """
    def __init__(
            self,
...
class ReplayBufferLSTMTD3:
    ...
    def sample_buffer(self, indices: np.ndarray):
        ...
        for i, index in enumerate(indices):
            ...
            #
            # note:
            # it seems like wrap around is not being handled for a smaller (circular array) replay memory buffer
            ...
            # tensor = torch.tensor(done, dtype=torch.float)  # tensor([0., 1., 0., 1., 0.])
            # terminal_indices = torch.where(tensor[1:5] == 1)  # (tensor([0, 2]),)
            # terminal_found = len(terminal_indices[0]) != 0  # True
            # last_terminal_index = terminal_indices[0][-1]  # tensor(2)
