...
Description:
    ...
    In general, TD3 is a superior version of DDPG and should perform clearly better due to,
        * Delayed target network updates
        * Usage of two critic networks to reduce overestimation bias (similar to double q-learning)

    Most of this code was adapted from following the free TD3 Tutorial by Dr. Phil Tabor.
    I then changed it to work with a multi-agent algorithm MADDPG to be MATD3.
    https://www.youtube.com/watch?v=ZhFO8EWADmY
...
class TD3:
    """
    Twin Delayed Deep Deterministic Policy Gradient (TD3) Reinforcement Learning Agent
    """
    def __init__(
            self,
            name: str,
    ...
    def learn(
        ...
        if self.optimization_step_counter % self.delay_interval == 0:
            ...
            # Jordan Note:
            # I think that I made a mistake here originally, but the algorithm was still able to learn.
            # The mistake is that these actions 'mu' should be calculated with the current observations,
            # not the next observations. I will change it to the current observations, but leave the
            # old code here for now.
            #
            # all agents' next actions are calculated for the batch and concatenated
            # mu = [agent.actor.forward(next_observations[index]) for index, agent in enumerate(agent_list)]

            # all agents' next actions are calculated for the batch and concatenated
            mu = [agent.actor.forward(observations[index]) for index, agent in enumerate(agent_list)]
